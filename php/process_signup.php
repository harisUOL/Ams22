...
/************ SEARCH ***********/
$con = connectTo();
$exists = $con->query("select * from `id19878262_amsdatabase`.`teacher` where email = '$email'");
if($exists && $con->affected_rows) {
  $con->close();
  respond("error","exists");
} 
/*********** INSERT ************/
$addTeacher = $con->query("insert into `id19878262_amsdatabase`.`teacher` (`name`, `email`, `phone`, `password`) values ('$name', '$email', '$phone', '$pass')");
